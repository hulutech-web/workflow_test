...
func (r *M20240624000000CreateWorkflowBaseTables) Up() error {
	...
	if err := facades.Schema().Create("flowlinks", func(table schema.Blueprint) {
		...
		table.String("type").Default("").Comment("类型: Condition=条件流转, Emp=指定人员, Dept=指定部门, Sys=系统自动")
		table.String("auditor").Default("0").Comment("审批人设置: -1000=发起人, -1001=部门主管, -1002=部门经理, -1003=表单字段, -1004=动态表达式")
		table.Text("expression").Nullable().Comment("条件判断表达式, 1表示无条件通过")
		table.String("condition_source").Default("").Comment("条件来源 前端/后端")
		table.Integer("sort").Default(0).Comment("条件判断顺序")
