<template>
  <div class="p-2">
    ...
    <vxe-grid ref='xGrid' v-bind="gridOptions" v-on="gridEvent">
      ...
      <template #design="{ row }">
        <a-button type="primary" size="small" @click="toDesign(row.id)">
          <PartitionOutlined/>
          管理流程图
        </a-button>
      </template>
      <template #action="{ row }">
        <a-button-group>
          <a-button type="primary" size="small" @click="editFlow(row)">编辑</a-button>
          <a-popconfirm title="确定要删除该流程吗？" ok-text="删除" cancel-text="取消"
                        ok-type="danger" @confirm="deleteFlow(row)">
            <a-button type="primary" danger size="small">删除</a-button>
          </a-popconfirm>
          <a-button type="primary" size="small" @click="startFlow(row)" :disabled="row.is_publish == false">
            <FormatPainterOutlined/>
            发起流程
          </a-button>
          <a-button type="primary" size="small" @click="startPlugin(row)">
            <ApiOutlined/>
            插件
          </a-button>
        </a-button-group>
