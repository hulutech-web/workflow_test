...
export default () => {
  // 方法
  const loadCommandOpt = async (parameter) => {
    return await http.request({
      url: `/command/read/options?parameter=${parameter}`,
      method: "GET",
    });
  };
  ...
  // 随机指令
  const getRandCommand = async (parameter) => {
    return await http.request({
      url: `command/rand`,
      method: "GET",
    });
  }
  const sendMainCommand = async (data) => {
    return await http.request({
      url: `/command/read/main`,
      method: "POST",
      data: data,
    });
}
