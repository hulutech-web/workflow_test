...
<template>
  <div>
    <div class='logo'>
      <img src="@/assets/logo/workflow.png" style="width:44px;" alt="">
      <!-- 使用 transition 包裹，监听展开状态 -->
      <transition name="fade-slide" :duration="{ enter: 900, leave: 0 }">
        <span v-if="active" class="sys-title">WFlow</span>
      </transition>
    </div>
...
.sys-title {
  ...
  font-weight: 700;
  /* 渐变文字 */
  background: linear-gradient(90deg, #0bace6, #a855f7, #4facfe, #0bace6);
  background-size: 300% 100%;
  -webkit-background-clip: text;
  -webkit-text-fill-color: transparent;
  background-clip: text;
  filter: drop-shadow(0 0 8px rgba(11, 172, 230, 0.4));
  /* 流动光效（永久） */
...
.fade-slide-enter-active {
  animation: coolAppear 0.9s cubic-bezier(0.34, 1.56, 0.64, 1) forwards;
}
...
.fade-slide-leave-active {
  ...
  opacity: 0 !important;
  transform: translateX(-30px) scale(0.7) rotate(-3deg) !important;
...
@keyframes coolAppear {
  0% {
    opacity: 0;
    transform: translateX(-30px) scale(0.7) rotate(-3deg);
    filter: blur(6px);
  }
  40% {
    opacity: 0.7;
  ...
  70% {
    transform: translateX(-3px) scale(0.98) rotate(0deg);
  }
  100% {
    opacity: 1;
    transform: translateX(0) scale(1) rotate(0deg);
    filter: blur(0) drop-shadow(0 0 8px rgba(11, 172, 230, 0.4)); /* 恢复发光 */
  }
}
</style>